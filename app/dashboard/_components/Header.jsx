...
function Header() {
  ...
  useEffect(() => {
    ...
    // for mobiles Prevent scrolling when menu is open
  const toggleMobileMenu = () => {
    setIsMobileMenuOpen((prev) => !prev);

    // Prevent body scrolling when menu is open
    if (!isMobileMenuOpen) {
      document.body.style.overflow = "hidden"; //does not allow user to scroll
    } else {
      document.body.style.overflow = "unset"; //allows user to scroll
    }
  };

   // closes the menu when user clicks a link or outside the menu
  ...
  return (
    <>
      ...
      >
        ...
        {/* Logo */}
        <Link
          href="/" /*redirect to home page when clicked */
          className="flex items-center gap-2"
          aria-label="PREP AI Interview Home"
          onClick={closeMobileMenu}
        >
          <Image src={"/artificial-intelligence.png"} width={50} height={40} alt="app logo" />
